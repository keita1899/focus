...
export default function AnnualRoadmapClient({ initialValue, birthday = "" }: { initialValue: unknown; birthday?: string }) {
  ...
  return <main className="shell roadmapPage annualRoadmapPage">
    <section className="annualRoadmapForm"><label>タイトル<input value={yearPlan.title} placeholder="この年のロードマップ" onChange={(event) => updateYear({ title: event.target.value })} /></label><div className="annualRoadmapToolbar"><div className="annualRoadmapYearSwitcher"><button type="button" onClick={() => changeYear(-1)} aria-label="前年へ">&lt;</button><strong>{selectedYear}年</strong>{age !== null && <span>{age}歳</span>}<button type="button" onClick={() => changeYear(1)} aria-label="翌年へ">&gt;</button></div></div><fieldset><legend>年間テーマ</legend>{yearPlan.themes.map((theme, index) => <input key={index} value={theme} placeholder={`テーマ ${index + 1}`} onChange={(event) => { const themes = [...yearPlan.themes]; themes[index] = event.target.value; updateYear({ themes }); }} />)}</fieldset></section>
    <div className="annualRoadmapMonths">{orderedMonths.map((monthNumber) => { const month = String(monthNumber); const label = `${month}月`; const plan = yearPlan.months[month]; const isOpen = Boolean(openMonths[month]); return <section className="annualRoadmapMonth" key={month}><button className="annualRoadmapMonthHeader" type="button" onClick={() => setOpenMonths((current) => ({ ...current, [month]: !isOpen }))} aria-expanded={isOpen}><strong>{label}</strong><span>{plan.theme || "月間テーマを入力"}</span><b>{isOpen ? "⌃" : "⌄"}</b></button>{isOpen && <div className="annualRoadmapMonthBody"><label>月間テーマ<input value={plan.theme} placeholder={`${label}のテーマ`} onChange={(event) => updateMonth(month, { theme: event.target.value })} /></label>{taskGroup(month, "mustDo")}<div className="annualRoadmapTaskColumns">{taskGroup(month, "chores")}{taskGroup(month, "other")}</div></div>}</section>; })}</div>
  </main>;
}
