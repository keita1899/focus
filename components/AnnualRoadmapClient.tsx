...
export default function AnnualRoadmapClient({ initialValue, birthday = "" }: { initialValue: unknown; birthday?: string }) {
  ...
  const [openMonths, setOpenMonths] = useState<Record<string, boolean>>({ "1": true });
  ...
  return <main className="shell roadmapPage annualRoadmapPage">
    ...
    <section className="annualRoadmapForm"><label>タイトル<input value={yearPlan.title} placeholder="この年のロードマップ" onChange={(event) => updateYear({ title: event.target.value })} /></label><fieldset><legend>その年のテーマ</legend>{yearPlan.themes.map((theme, index) => <input key={index} value={theme} placeholder={`テーマ ${index + 1}`} onChange={(event) => { const themes = [...yearPlan.themes]; themes[index] = event.target.value; updateYear({ themes }); }} />)}</fieldset></section>
    <div className="annualRoadmapMonths">{monthLabels.map((label, index) => { const month = String(index + 1); const plan = yearPlan.months[month]; const isOpen = Boolean(openMonths[month]); return <section className="annualRoadmapMonth" key={month}><button className="annualRoadmapMonthHeader" type="button" onClick={() => setOpenMonths((current) => ({ ...current, [month]: !isOpen }))} aria-expanded={isOpen}><strong>{label}</strong><span>{plan.theme || "月のテーマを入力"}</span><b>{isOpen ? "⌃" : "⌄"}</b></button>{isOpen && <div className="annualRoadmapMonthBody"><label>月のテーマ<input value={plan.theme} placeholder={`${label}のテーマ`} onChange={(event) => updateMonth(month, { theme: event.target.value })} /></label>{(["mustDo", "chores", "other"] as TaskKind[]).map((kind) => <section className="annualRoadmapTaskGroup" key={kind}><h3>{taskLabels[kind]}</h3>{plan[kind].map((task, taskIndex) => <div key={taskIndex}><input value={task} placeholder={taskLabels[kind]} onChange={(event) => updateTask(month, kind, taskIndex, event.target.value)} /><button type="button" onClick={() => removeTask(month, kind, taskIndex)} aria-label={`${taskLabels[kind]}を削除`}>×</button></div>)}<button className="annualRoadmapAddTask" type="button" onClick={() => addTask(month, kind)}>＋ 追加</button></section>)}</div>}</section>; })}</div>
  </main>;
}
