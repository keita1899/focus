...
function TaskAddControl({ controlKey, activeKey, label, placeholder, onOpen, onAdd }: { controlKey: string; activeKey: string | null; label: string; placeholder: string; onOpen: (key: string) => void; onAdd: (title: string) => void }) {
  ...
  if (activeKey !== controlKey) return <button className="annualRoadmapAddTask" type="button" onClick={() => onOpen(controlKey)}>＋ {label}</button>;
  return <form className="annualRoadmapAddForm" onSubmit={submit}>
    <input ref={inputRef} autoFocus value={title} onChange={(event) => setTitle(event.currentTarget.value)} placeholder={placeholder} aria-label={placeholder} />
...
export default function AnnualRoadmapClient({ initialValue, birthday = "", onStateChange }: { initialValue: unknown; birthday?: string; onStateChange?: (value: unknown) => void }) {
  ...
  const [openTaskFormKey, setOpenTaskFormKey] = useState<string | null>(null);
  ...
  function updateMustDo(month: string, id: string, value: Partial<RoadmapTask>, parentId?: string) { if ("done" in value && !parentId) { removeMustDo(month, id); return; } const tasks = yearPlan.months[month].mustDo.map((task) => parentId ? task.id === parentId ? { ...task, children: task.children.map((child) => child.id === id ? { ...child, ...value } : child) } : task : task.id === id ? { ...task, ...value } : task); updateMonth(month, { mustDo: tasks }); }
  ...
  function taskGroup(month: string, kind: TaskKind) {
    if (kind === "mustDo") {
      ...
      return <section className="annualRoadmapTaskGroup annualRoadmapMustDoGroup" key={kind}><h3>{taskLabels[kind]}</h3>{tasks.map((task) => {
        ...
        return <div className="annualRoadmapParentTask" key={task.id}><div className={`annualRoadmapTask annualRoadmapScheduledTask${task.done ? " done" : ""}`}><button className="roadmapTaskCompleteButton" type="button" onClick={() => updateMustDo(month, task.id, { done: !task.done })} aria-label={`${task.title || "やるべきこと"}の完了を切り替え`} />{task.children.length > 0 ? <button className="annualRoadmapParentToggle" type="button" onClick={() => setCollapsedParents((current) => ({ ...current, [task.id]: !collapsed }))} aria-label={`子タスクを開閉、${completedChildren}/${task.children.length}件完了`}><span>{collapsed ? "›" : "⌄"}</span><small>{completedChildren}/{task.children.length}</small></button> : <span aria-hidden="true" />}<input data-roadmap-task-id={task.id} value={task.title} placeholder="やるべきこと" onChange={(event) => updateMustDo(month, task.id, { title: event.target.value })} /><TaskSchedulePicker task={task} isOpen={openScheduleTaskId === scheduleKey} onOpenChange={(open) => setOpenScheduleTaskId(open ? scheduleKey : null)} onChange={(value) => updateMustDo(month, task.id, value)} /><button type="button" onClick={() => removeMustDo(month, task.id)} aria-label="やるべきことを削除">×</button></div>{!collapsed && <div className="annualRoadmapChildTasks">{sortTasksBySchedule(task.children).map((child) => { const childScheduleKey = `${scheduleKey}-child-${child.id}`; return <div className={`annualRoadmapTask annualRoadmapScheduledTask${child.done ? " done" : ""}`} key={child.id}><button className="roadmapTaskCompleteButton" type="button" onClick={() => updateMustDo(month, child.id, { done: !child.done }, task.id)} aria-label={`${child.title || "子タスク"}の完了を切り替え`} /><span aria-hidden="true" /><input data-roadmap-task-id={child.id} value={child.title} placeholder="子タスク" onChange={(event) => updateMustDo(month, child.id, { title: event.target.value }, task.id)} /><TaskSchedulePicker task={child} isOpen={openScheduleTaskId === childScheduleKey} onOpenChange={(open) => setOpenScheduleTaskId(open ? childScheduleKey : null)} onChange={(value) => updateMustDo(month, child.id, value, task.id)} /><button type="button" onClick={() => removeMustDo(month, child.id, task.id)} aria-label="子タスクを削除">×</button></div>; })}<TaskAddControl controlKey={childFormKey} activeKey={openTaskFormKey} label="子タスクを追加" placeholder="子タスク" onOpen={setOpenTaskFormKey} onAdd={(title) => addMustDo(month, title, task.id)} /></div>}</div>;
      })}<TaskAddControl controlKey={`${selectedYear}-${month}-${kind}-root`} activeKey={openTaskFormKey} label="タスクを追加" placeholder="やるべきこと" onOpen={setOpenTaskFormKey} onAdd={(title) => addMustDo(month, title)} /></section>;
    }
    const tasks = sortTasksBySchedule(yearPlan.months[month][kind]);
    const formKey = `${selectedYear}-${month}-${kind}-root`;
