...
export default function AnnualRoadmapClient({ initialValue, birthday = "", onStateChange }: { initialValue: unknown; birthday?: string; onStateChange?: (value: unknown) => void }) {
  ...
  function taskGroup(month: string, kind: TaskKind) {
    if (kind === "mustDo") {
      ...
      return <section className="annualRoadmapTaskGroup annualRoadmapMustDoGroup" key={kind}><h3>{taskLabels[kind]}</h3>{tasks.map((task) => <div className="annualRoadmapParentTask" key={task.id}><div className="annualRoadmapTask annualRoadmapScheduledTask"><span aria-hidden="true" /><input data-roadmap-task-id={task.id} value={task.title} placeholder="やるべきこと" onKeyDown={(event) => { if (event.key === "Enter") { event.preventDefault(); addMustDo(month); } }} onChange={(event) => updateMustDo(month, task.id, { title: event.target.value })} /><input type="date" value={task.scheduledDate || ""} onChange={(event) => updateMustDo(month, task.id, { scheduledDate: event.target.value || undefined })} aria-label="予定日" /><input type="time" value={task.scheduledTime || ""} onChange={(event) => updateMustDo(month, task.id, { scheduledTime: event.target.value || undefined })} aria-label="予定時刻" /><button type="button" onClick={() => removeMustDo(month, task.id)} aria-label="やるべきことを削除">×</button></div><div className="annualRoadmapChildTasks">{task.children.map((child) => <div className="annualRoadmapTask annualRoadmapScheduledTask" key={child.id}><span aria-hidden="true" /><input data-roadmap-task-id={child.id} value={child.title} placeholder="子タスク" onKeyDown={(event) => { if (event.key === "Enter") { event.preventDefault(); addMustDo(month, task.id); } }} onChange={(event) => updateMustDo(month, child.id, { title: event.target.value }, task.id)} /><input type="date" value={child.scheduledDate || ""} onChange={(event) => updateMustDo(month, child.id, { scheduledDate: event.target.value || undefined }, task.id)} aria-label="予定日" /><input type="time" value={child.scheduledTime || ""} onChange={(event) => updateMustDo(month, child.id, { scheduledTime: event.target.value || undefined }, task.id)} aria-label="予定時刻" /><button type="button" onClick={() => removeMustDo(month, child.id, task.id)} aria-label="子タスクを削除">×</button></div>)}<button className="annualRoadmapAddChildTask" type="button" onClick={() => addMustDo(month, task.id)}>＋ 子タスクを追加</button></div></div>)}<button className="annualRoadmapAddTask" type="button" onClick={() => addMustDo(month)}>＋ タスクを追加</button></section>;
    }
