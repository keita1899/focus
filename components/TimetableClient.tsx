...
export default function TimetableClient({ initialPlannerValue }: TimetableClientProps) {
  const initialPlanner = normalizePlanner(initialPlannerValue);
  const [planner, setPlanner] = useState<PlannerLike>(initialPlanner);
  const [isReady, setIsReady] = useState(initialPlannerValue !== null);
  const [activeTimetableId, setActiveTimetableId] = useState(
    initialPlanner.timetables?.[0]?.id || "",
  );
