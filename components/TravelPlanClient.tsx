...
export default function TravelPlanClient({ initialValue, tripId }: { initialValue: unknown; tripId: string }) {
  ...
  const addEntry = (event: FormEvent<HTMLFormElement>, dayIndex: number) => {
    ...
    const entry = { id: `travel-entry-${Date.now()}`, type, title, cost: Math.max(0, Number(form.get("cost")) || 0), ...(startTime ? { startTime } : {}), ...(endTime ? { endTime } : {}), ...(type === "transport" ? { transportMode: String(form.get("transportMode") || transportModes[0]) } : {}) };
