...
export default function TravelPlanClient({ initialValue, tripId }: { initialValue: unknown; tripId: string }) {
  ...
  const addChecklistItem = (event: FormEvent<HTMLFormElement>, category: TravelChecklistCategory) => {
    event.preventDefault();
    const form = new FormData(event.currentTarget);
    const text = String(form.get("checklistItem") || "").trim();
    if (!text) return;
    updateTrip((current) => ({ ...current, checklist: [...current.checklist, { id: `travel-check-${Date.now()}`, text, done: false, category }] }));
    event.currentTarget.reset();
  };
  ...
  const checklistGroups: Array<{ key: TravelChecklistCategory; label: string }> = [{ key: "before", label: "旅行前" }, { key: "during", label: "旅行中" }, { key: "souvenir", label: "お土産" }];
