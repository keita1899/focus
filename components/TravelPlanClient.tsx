...
export default function TravelPlanClient({ initialValue, tripId }: { initialValue: unknown; tripId: string }) {
  ...
  return <main className="shell travelPage">
    ...
    <div className="travelDays">{visibleDays.map(({ day, index: dayIndex }) => { const selectedType = entryTypes[dayIndex] || "activity"; const dayTotal = day.entries.reduce((sum, entry) => sum + entry.cost, 0); return <section className="travelDayCard" key={day.date}>
      ...
      <div className="travelEntryList">{day.entries.length === 0 ? <p className="emptyText">まだ予定がありません。</p> : day.entries.map((entry) => <article className={`travelEntry travelEntry-${entry.type}`} key={entry.id} onDoubleClick={() => setEditTarget({ dayIndex, entry })} title="ダブルクリックで編集"><span className="travelEntryType">{entryTypeLabels[entry.type]}</span><div><strong>{entry.title}</strong>{(entry.startTime || entry.endTime || entry.transportMode) && <small>{[entry.startTime && entry.endTime ? `${entry.startTime}〜${entry.endTime}` : entry.startTime ? `${entry.startTime}〜` : entry.endTime ? `〜${entry.endTime}` : "", entry.transportMode ? `${transportModeIcons[entry.transportMode] || ""} ${entry.transportMode}` : ""].filter(Boolean).join(" ・ ")}</small>}</div><b>¥{entry.cost.toLocaleString()}</b><button type="button" onClick={() => removeEntry(dayIndex, entry.id)} aria-label={`${entry.title}を削除`}>×</button></article>)}</div>
      <form className="travelEntryForm" onSubmit={(event) => addEntry(event, dayIndex)}><select name="type" value={selectedType} onChange={(event) => { const nextType = event.currentTarget.value as TravelEntryType; setEntryTypes((current) => ({ ...current, [dayIndex]: nextType })); }}><option value="activity">目的</option><option value="transport">移動</option><option value="meal">ごはん</option></select>{selectedType === "transport" && transportSelect()}{selectedType === "transport" ? <div className="travelRouteInputs"><input name="origin" placeholder="出発地" required /><span>→</span><input name="destination" placeholder="到着地" required /></div> : <input className="travelEntryTitleInput" name="title" placeholder={selectedType === "meal" ? "お店・食事" : "場所・やること"} required />}<div className="travelTimeRange"><input name="startTime" type="time" aria-label="開始時刻" /><span>〜</span><input name="endTime" type="time" aria-label="終了時刻" /></div><label className="travelCostInput"><span>¥</span><input name="cost" type="number" min="0" inputMode="numeric" placeholder="費用" /></label><button type="submit">追加</button></form>
    </section>; })}</div>
    {editTarget && <div className="travelModalBackdrop" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) setEditTarget(null); }}><section className="travelEditModal" role="dialog" aria-modal="true" aria-label="予定を編集"><header><h2>予定を編集</h2><button type="button" onClick={() => setEditTarget(null)} aria-label="閉じる">×</button></header><form onSubmit={editEntry}><input type="hidden" name="type" value={editTarget.entry.type} />{editTarget.entry.type === "transport" && transportSelect(editTarget.entry.transportMode)}{editTarget.entry.type === "transport" ? <div className="travelRouteInputs"><input name="origin" defaultValue={editTarget.entry.origin || editTarget.entry.title.split(" → ")[0] || ""} placeholder="出発地" required /><span>→</span><input name="destination" defaultValue={editTarget.entry.destination || editTarget.entry.title.split(" → ")[1] || ""} placeholder="到着地" required /></div> : <input name="title" defaultValue={editTarget.entry.title} placeholder="内容" required />}<div className="travelTimeRange"><input name="startTime" type="time" defaultValue={editTarget.entry.startTime || ""} aria-label="開始時刻" /><span>〜</span><input name="endTime" type="time" defaultValue={editTarget.entry.endTime || ""} aria-label="終了時刻" /></div><label className="travelCostInput"><span>¥</span><input name="cost" type="number" min="0" defaultValue={editTarget.entry.cost} inputMode="numeric" /></label><div className="travelEditActions"><button type="button" onClick={() => setEditTarget(null)}>キャンセル</button><button type="submit">保存</button></div></form></section></div>}
  </main>;
}
