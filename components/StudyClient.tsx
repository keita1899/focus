...
export default function StudyClient({ initialValue }: { initialValue: unknown }) {
  ...
  const [journalRows, setJournalRows] = useState<JournalRow[]>([{ id: "journal-1", debitAccount: "", debitAmount: "", creditAccount: "", creditAmount: "" }]);
  ...
  const journalAnswer = journalRows.filter((row) => row.debitAccount.trim() || row.creditAccount.trim()).map((row) => `借方: ${row.debitAccount || "－"}${row.debitAmount ? ` ${Number(row.debitAmount).toLocaleString()}円` : ""} / 貸方: ${row.creditAccount || "－"}${row.creditAmount ? ` ${Number(row.creditAmount).toLocaleString()}円` : ""}`).join("\n");
  ...
  const openReview = (id: string) => { setReviewAnswer(""); setReviewAnswerMode("text"); setJournalRows([{ id: `journal-${Date.now()}`, debitAccount: "", debitAmount: "", creditAccount: "", creditAmount: "" }]); setShowReferenceAnswer(false); setOpenAnswerCardId(id); };
