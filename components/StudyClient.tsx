...
const emptyJournalRow = (): JournalRow => ({ id: `journal-${Date.now()}-${Math.random()}`, debitAccount: "", debitAmount: "", creditAccount: "", creditAmount: "" });
const formatJournal = (rows: JournalRow[]) => rows.filter((row) => row.debitAccount.trim() || row.creditAccount.trim()).map((row) => `借方: ${row.debitAccount || "－"}${row.debitAmount ? ` ${Number(row.debitAmount).toLocaleString()}円` : ""} / 貸方: ${row.creditAccount || "－"}${row.creditAmount ? ` ${Number(row.creditAmount).toLocaleString()}円` : ""}`).join("\n");

function JournalEditor({ rows, onChange }: { rows: JournalRow[]; onChange: (rows: JournalRow[]) => void }) {
  const update = (id: string, value: Partial<JournalRow>) => onChange(rows.map((row) => row.id === id ? { ...row, ...value } : row));
  return <div className="studyJournalForm">{rows.map((row) => <div className="studyJournalRow" key={row.id}><div><span>借方</span><input value={row.debitAccount} onChange={(event) => update(row.id, { debitAccount: event.currentTarget.value })} placeholder="勘定科目" /><input type="number" min="0" value={row.debitAmount} onChange={(event) => update(row.id, { debitAmount: event.currentTarget.value })} placeholder="金額" /></div><div><span>貸方</span><input value={row.creditAccount} onChange={(event) => update(row.id, { creditAccount: event.currentTarget.value })} placeholder="勘定科目" /><input type="number" min="0" value={row.creditAmount} onChange={(event) => update(row.id, { creditAmount: event.currentTarget.value })} placeholder="金額" /></div>{rows.length > 1 && <button type="button" onClick={() => onChange(rows.filter((item) => item.id !== row.id))} aria-label="仕訳行を削除">×</button>}</div>)}<button type="button" onClick={() => onChange([...rows, emptyJournalRow()])}>＋ 勘定科目を追加</button></div>;
}
...
function normalize(value: unknown): State {
  ...
  const cards = Array.isArray(source.cards)
    ? source.cards.filter((card): card is Card => Boolean(card) && typeof card.id === "string").map((card) => ({
      ...card,
      answer: card.answer || "",
      answerMode: (card.answerMode === "journal" ? "journal" : "text") as Card["answerMode"],
      answerJournalRows: Array.isArray(card.answerJournalRows) ? card.answerJournalRows : [],
      notes: card.notes || "",
      source: card.source || "",
      material: card.material || card.source || "",
      page: card.page || "",
      problemNumber: card.problemNumber || "",
...
export default function StudyClient({ initialValue }: { initialValue: unknown }) {
  ...
  const [reviewAnswerMode, setReviewAnswerMode] = useState<"text" | "journal">("text");
  const [journalRows, setJournalRows] = useState<JournalRow[]>([emptyJournalRow()]);
  const [answerMode, setAnswerMode] = useState<"text" | "journal">("text");
  const [answerJournalRows, setAnswerJournalRows] = useState<JournalRow[]>([emptyJournalRow()]);
  const [editAnswerMode, setEditAnswerMode] = useState<"text" | "journal">("text");
  const [editAnswerJournalRows, setEditAnswerJournalRows] = useState<JournalRow[]>([emptyJournalRow()]);
  ...
  const visibleGroups = visible.reduce<Array<{ date: string; cards: Card[] }>>((groups, card) => {
    const createdDate = card.createdAt.slice(0, 10);
    const currentGroup = groups.at(-1);
    if (currentGroup?.date === createdDate) currentGroup.cards.push(card);
    else groups.push({ date: createdDate, cards: [card] });
    return groups;
  }, []);
  ...
  const openReview = (id: string) => { setReviewAnswer(""); setReviewAnswerMode("text"); setJournalRows([emptyJournalRow()]); setShowReferenceAnswer(false); setOpenAnswerCardId(id); };
  const openEdit = (card: Card) => { setEditAnswerMode(card.answerMode); setEditAnswerJournalRows(card.answerJournalRows.length ? card.answerJournalRows : [emptyJournalRow()]); setEditingCardId(card.id); };
  const review = (id: string, level: Level) => {
    const answer = currentReviewAnswer;
    if (!answer) return;
  ...
  const updateJournalRow = (id: string, value: Partial<JournalRow>) => setJournalRows((current) => current.map((row) => row.id === id ? { ...row, ...value } : row));
  ...
  const editCard = (event: FormEvent<HTMLFormElement>, id: string) => {
    ...
    if (!question) return;
    const answer = editAnswerMode === "journal" ? formatJournal(editAnswerJournalRows) : String(form.get("answer") || "");
    const material = String(form.get("material") || "");
    const page = String(form.get("page") || "");
    const problemNumber = String(form.get("problemNumber") || "");
    const source = [material, page && `${page}ページ`, problemNumber && `問題 ${problemNumber}`].filter(Boolean).join(" / ");
    setState((current) => ({ ...current, cards: current.cards.map((item) => item.id === id ? { ...item, question, answer, answerMode: editAnswerMode, answerJournalRows: editAnswerMode === "journal" ? editAnswerJournalRows : [], notes: String(form.get("notes") || ""), source, material, page, problemNumber, category: String(form.get("category") || item.category) } : item) }));
  ...
  const addCard = (event: FormEvent<HTMLFormElement>) => {
    ...
    const question = String(form.get("question") || "").trim();
    if (!question) return;
    const answer = answerMode === "journal" ? formatJournal(answerJournalRows) : String(form.get("answer") || "");
    const material = String(form.get("material") || "");
    const page = String(form.get("page") || "");
    const problemNumber = String(form.get("problemNumber") || "");
    const source = [material, page && `${page}ページ`, problemNumber && `問題 ${problemNumber}`].filter(Boolean).join(" / ");
    setState((current) => ({ ...current, cards: [...current.cards, { id: `study-${Date.now()}`, question, answer, answerMode, answerJournalRows: answerMode === "journal" ? answerJournalRows : [], notes: String(form.get("notes") || ""), source, material, page, problemNumber, category: String(form.get("category") || current.categories[0] || "未分類"), dueDate: date(), level: 0, createdAt: now(), reviewAttempts: [] }] }));
    event.currentTarget.reset(); setAnswerMode("text"); setAnswerJournalRows([emptyJournalRow()]); setView("list");
  };
  ...
  return <main className="studyShell">
    ...
    {section === "review" ? <div className="studyLayout">
      ...
      <div className="studyMain">
        {view === "list" && <section className="studyQuestionSection"><div className="studyQuestionHeading"><h2>{category}</h2><span>{visible.length}件</span></div><div className="studyListControls"><label>並び順<select value={sortOrder} onChange={(event) => setSortOrder(event.currentTarget.value as SortOrder)}><option value="newest">新しい順</option><option value="oldest">古い順</option></select></label><label>理解度<select value={levelFilter} onChange={(event) => setLevelFilter(event.currentTarget.value as LevelFilter)}><option value="all">すべて</option><option value="0">要復習</option><option value="1">理解中</option><option value="2">定着</option></select></label></div>{visible.length === 0 ? <p className="emptyText">条件に合う問題はありません。</p> : <div className="studyQuestionGrid">{visibleGroups.map((group) => <section className="studyDateGroup" key={group.date}><div className="studyDateDivider"><span>{displayDate(`${group.date}T00:00:00`)}</span></div>{group.cards.map((item) => <article className="studyCard studyQuestionCard" key={item.id}><div className="studyQuestionCardActions"><button type="button" onClick={() => openEdit(item)} aria-label={`${item.question}を編集`}><svg aria-hidden="true" viewBox="0 0 24 24"><path d="m4 20 4.5-1 10-10a2.1 2.1 0 0 0-3-3l-10 10L4 20Z"/><path d="m14 7 3 3"/></svg></button><button type="button" onClick={() => removeCard(item.id)} aria-label={`${item.question}を削除`}>×</button></div><div className="studyCardMeta"><span>{item.category}</span><span className={`studyLevel level-${item.level}`}>{levelLabels[item.level]}</span><small>作成日 {displayDate(item.createdAt)}</small></div><h3>{item.question}</h3>{item.source && <small>{item.source}</small>}<button className="studyReveal" type="button" onClick={() => openReview(item.id)}>復習する</button></article>)}</section>)}</div>}</section>}
        {view === "add" && <form className="studyCard studyAddForm" onSubmit={addCard}><h2>問題を追加</h2><label>カテゴリー<select name="category" defaultValue={category === "すべて" ? state.categories[0] : category}>{state.categories.map((item) => <option key={item} value={item}>{item}</option>)}</select></label><label>問題・論点名<input name="question" required /></label><div className="studyAnswerInput"><div className="studyReviewAnswerHeading"><strong>解答・要点</strong><div><button className={answerMode === "text" ? "active" : undefined} type="button" onClick={() => setAnswerMode("text")}>記述</button><button className={answerMode === "journal" ? "active" : undefined} type="button" onClick={() => setAnswerMode("journal")}>仕訳</button></div></div>{answerMode === "text" ? <textarea name="answer" /> : <JournalEditor rows={answerJournalRows} onChange={setAnswerJournalRows} />}</div><label>解き方・注意点<textarea name="notes" /></label><div className="studyReferenceFields"><label>教材<input name="material" /></label><label>ページ<input name="page" inputMode="numeric" /></label><label>問題番号<input name="problemNumber" /></label></div><button>追加して今日復習する</button></form>}
    ...
    {editingCard && <div className="studyAnswerModalBackdrop" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) setEditingCardId(null); }}><section className="studyAnswerModal studyEditModal" role="dialog" aria-modal="true" aria-labelledby={`study-edit-${editingCard.id}`}><div className="studyModalHeading"><h2 id={`study-edit-${editingCard.id}`}>問題を編集</h2><button type="button" onClick={() => setEditingCardId(null)} aria-label="閉じる">×</button></div><form className="studyAddForm" onSubmit={(event) => editCard(event, editingCard.id)}><label>カテゴリー<select name="category" defaultValue={editingCard.category}>{state.categories.map((name) => <option key={name} value={name}>{name}</option>)}</select></label><label>問題・論点名<input name="question" defaultValue={editingCard.question} required /></label><div className="studyAnswerInput"><div className="studyReviewAnswerHeading"><strong>解答・要点</strong><div><button className={editAnswerMode === "text" ? "active" : undefined} type="button" onClick={() => setEditAnswerMode("text")}>記述</button><button className={editAnswerMode === "journal" ? "active" : undefined} type="button" onClick={() => setEditAnswerMode("journal")}>仕訳</button></div></div>{editAnswerMode === "text" ? <textarea name="answer" defaultValue={editingCard.answer} ref={(node) => { if (node) resizeTextarea(node); }} onInput={(event) => resizeTextarea(event.currentTarget)} /> : <JournalEditor rows={editAnswerJournalRows} onChange={setEditAnswerJournalRows} />}</div><label>解き方・注意点<textarea name="notes" defaultValue={editingCard.notes} ref={(node) => { if (node) resizeTextarea(node); }} onInput={(event) => resizeTextarea(event.currentTarget)} /></label><div className="studyReferenceFields"><label>教材<input name="material" defaultValue={editingCard.material} /></label><label>ページ<input name="page" inputMode="numeric" defaultValue={editingCard.page} /></label><label>問題番号<input name="problemNumber" defaultValue={editingCard.problemNumber} /></label></div><div className="studyEditActions"><button type="button" onClick={() => setEditingCardId(null)}>キャンセル</button><button type="submit">保存</button></div></form></section></div>}
    {openAnswerCard && <div className="studyAnswerModalBackdrop" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) setOpenAnswerCardId(null); }}><section className="studyAnswerModal" role="dialog" aria-modal="true" aria-labelledby={`study-answer-${openAnswerCard.id}`}><header><div><span>{openAnswerCard.category}</span><h2 id={`study-answer-${openAnswerCard.id}`}>{openAnswerCard.question}</h2>{openAnswerCard.source && <small>{openAnswerCard.source}</small>}</div><button type="button" onClick={() => setOpenAnswerCardId(null)} aria-label="閉じる">×</button></header><div className="studyReviewAnswerHeading"><strong>今回の回答</strong><div><button className={reviewAnswerMode === "text" ? "active" : undefined} type="button" onClick={() => setReviewAnswerMode("text")}>記述</button><button className={reviewAnswerMode === "journal" ? "active" : undefined} type="button" onClick={() => setReviewAnswerMode("journal")}>仕訳</button></div></div>{reviewAnswerMode === "text" ? <label className="studyReviewAnswer"><textarea value={reviewAnswer} onChange={(event) => setReviewAnswer(event.currentTarget.value)} onInput={(event) => resizeTextarea(event.currentTarget)} placeholder="自分の答えを入力" autoFocus /></label> : <div className="studyJournalForm">{journalRows.map((row) => <div className="studyJournalRow" key={row.id}><div><span>借方</span><input value={row.debitAccount} onChange={(event) => updateJournalRow(row.id, { debitAccount: event.currentTarget.value })} placeholder="勘定科目" /><input type="number" min="0" value={row.debitAmount} onChange={(event) => updateJournalRow(row.id, { debitAmount: event.currentTarget.value })} placeholder="金額" /></div><div><span>貸方</span><input value={row.creditAccount} onChange={(event) => updateJournalRow(row.id, { creditAccount: event.currentTarget.value })} placeholder="勘定科目" /><input type="number" min="0" value={row.creditAmount} onChange={(event) => updateJournalRow(row.id, { creditAmount: event.currentTarget.value })} placeholder="金額" /></div>{journalRows.length > 1 && <button type="button" onClick={() => setJournalRows((current) => current.filter((item) => item.id !== row.id))} aria-label="仕訳行を削除">×</button>}</div>)}<button type="button" onClick={() => setJournalRows((current) => [...current, { id: `journal-${Date.now()}`, debitAccount: "", debitAmount: "", creditAccount: "", creditAmount: "" }])}>＋ 勘定科目を追加</button></div>}{!showReferenceAnswer ? <button className="studyCheckAnswer" type="button" disabled={!currentReviewAnswer} onClick={() => setShowReferenceAnswer(true)}>解答を確認</button> : <><div className="studyAnswerModalBody">{openAnswerCard.answer.trim() && <section className="studyAnswerSection"><h3>解答・要点</h3><p>{openAnswerCard.answer}</p></section>}{openAnswerCard.notes.trim() && <section className="studyAnswerSection"><h3>解き方・注意点</h3><p>{openAnswerCard.notes}</p></section>}{!openAnswerCard.answer.trim() && !openAnswerCard.notes.trim() && <p className="emptyText">解答・注意点は未入力です。</p>}</div><div className="studyLevelButtons"><button type="button" onClick={() => review(openAnswerCard.id, 0)}>要復習</button><button type="button" onClick={() => review(openAnswerCard.id, 1)}>理解中</button><button type="button" onClick={() => review(openAnswerCard.id, 2)}>定着</button></div></>}{openAnswerCard.reviewAttempts.length > 0 && <details className="studyAttemptHistory"><summary>過去の回答 {openAnswerCard.reviewAttempts.length}件</summary><div>{[...openAnswerCard.reviewAttempts].reverse().map((attempt) => <article key={`${attempt.answeredAt}-${attempt.answer}`}><small>{displayDateTime(attempt.answeredAt)}</small><p>{attempt.answer}</p></article>)}</div></details>}</section></div>}
  </main>;
}
