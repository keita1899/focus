...
type Card = { id: string; question: string; answer: string; answerMode: "text" | "journal"; answerJournalRows: JournalRow[]; notes: string; source: string; category: string; dueDate: string; level: Level; lastReviewed?: string; createdAt: string; reviewAttempts: ReviewAttempt[] };
...
export default function StudyClient({ initialValue }: { initialValue: unknown }) {
  ...
  const editCard = (event: FormEvent<HTMLFormElement>, id: string) => {
    ...
    setState((current) => ({ ...current, cards: current.cards.map((item) => item.id === id ? { ...item, question, answer, answerMode: editAnswerMode, answerJournalRows: editAnswerMode === "journal" ? editAnswerJournalRows : [], notes: String(form.get("notes") || ""), source: String(form.get("source") || ""), category: String(form.get("category") || item.category) } : item) }));
